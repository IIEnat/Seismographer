...
RMS is used in [playback_routes.py](../main/python/playback_routes.py) within `playback_stats()` to obtain a average signal amplitude for colour representation on the playback map.   The RMS value is computed by squaring each sample in the signal, averaging these squares over the window, and then taking the square root of the result.
